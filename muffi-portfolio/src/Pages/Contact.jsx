...
const Contact = () => {
  return (
      <Box id='contact'minH="50vh " w="90%" m="auto" pb='2rem'>
          ...
          <Heading fontFamily={'cursive'} fontWeight='md' textAlign={"center"}>Connect With Me</Heading>
          <Grid w={{base:"90vw",lg:"80vw"}} m="auto" mt='2rem' gridTemplateColumns={{base:"repeat(1,250px)",md:"repeat(2,300px)",lg:"repeat(4,1fr)"}} gap="2rem" alignContent={"center"} justifyContent='center'>
              {
                  contact.map((item,i) => (
                      <GridItem key={i}>
                              ...
                              <Text fontSize={"xl"} fontFamily='cursive'>{item.name}</Text>
                              <Link href={item.link} isExternal><Text fontSize={"lg"} fontFamily='cursive'>{ item.value}</Text>
                          </Link>
                          </Flex>
                      </GridItem>
                  ))
