...
const Skills = () => {
  ...
  return (
    <Flex id="skills" direction={"column"} gap='3rem' w="90%" m="auto" minH={"100vh"} >
      <Box  my="2rem">
      <Heading fontFamily={"cursive"}fontSize={{base:"2xl",md:"3xl",lg:"4xl"}} fontWeight="md">Professional Skills</Heading>
          ...
          skillDB.map((skill) => (
            <GridItem data-aos="fade-up" key={skill.id}>
              <Flex  direction={"column"} bgColor={"#151C28"} border="1px solid #77BEF8" boxShadow={" #77BEF8 0px 3px 8px"} alignItems={"center"} p="1rem" borderRadius={"10px"} gap='.5rem' _hover={{ color: "#77BEF8",transform:"scale(1.1,1.1)"}}>
                ...
                <Text fontFamily={"cursive"} fontSize={{base:"md",sm:"md",md:"lg",lg:"xl"}} textTransform={'capitalize'}>{skill.name}</Text>
              </Flex>
            </GridItem>
          ))
        }
      ...
      <Box  my="2rem">
      <Heading fontFamily={"cursive"} fontSize={{base:"2xl",md:"3xl",lg:"4xl"}} fontWeight="md">Tools I use</Heading>
          ...
          ToolsDB.map((tool) => (
            <GridItem data-aos="fade-up" key={tool.id} alignItems='center' justifyItems={"center"}>
              <Flex direction={"column"} bgColor={"#151C28"} border="1px solid #77BEF8" boxShadow={" #77BEF8 0px 3px 8px"} alignItems={"center"} p="1rem" borderRadius={"10px"} gap='.5rem' _hover={{ color: "#77BEF8",transform:"scale(1.1,1.1)"}}>
                ...
                <Text fontFamily={"cursive"}  fontSize={{base:"md",sm:"md",md:"lg",lg:"xl"}} textTransform={'capitalize'}>{tool.name}</Text>
              </Flex>
            </GridItem>
          ))
        }
